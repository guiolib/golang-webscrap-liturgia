...
func buscarLiturgia(dataMissa time.Time) LeituraLiturgia {
	...
	cabecalho := doc.Find(".blog-header").NextFiltered(".container")
	titulo = strings.TrimSpace(cabecalho.Find("h2").Text())
	cor = cabecalho.Find("p.text-right").Text()
	regexCor := regexp.MustCompile(":\\s?(\\w+)$")
	cor = regexCor.FindAllStringSubmatch(cor, 1)[0][1]
	...
	doc.Find("#corpo_leituras").Children().Each(func(i int, elm *goquery.Selection) {
		...
		if i == 1 {
			...
			prefixo = strings.TrimSpace(elm.Find(".title-leitura").Text())
			...
			sufixo = strings.TrimSpace(sufixo)
		} else {
			// for {
			elm.Find("div").Find("div").Each(func(iy int, elmInterno *goquery.Selection) {
				div := elmInterno
				// fmt.Println(" -- ", div.Text())
				if valor := strings.TrimSpace(div.Text()); strings.Index(valor, "Leitura") > -1 {
					prefixo = valor
				}
				if valor := strings.TrimSpace(div.Text()); prefixo == "" && strings.Index(valor, "Proclama") > -1 {
					prefixo = valor
				}
			...
			sufixo = strings.TrimSpace(elm.Find(".title-leitura").Text())
